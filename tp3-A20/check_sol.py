...
# ADDED: (timeout 180s ./tp.sh -e exemplaires_competition/100_100_m-500.txt -c 500 -p; exit 0) | python ./check_sol.py -e exemplaires_competition/100_100_m-500.txt -c 500
...
def parse_solution(raw_solution, do_transpose):
    ...
    for line in raw_solution.splitlines():
        ...
        if not line_contents and not single_solution:
            ...
            single_solution.append([raw_district[x:x+2]
                                    for x in range(0, len(raw_district), 2)])
            if do_transpose:
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Erreur : impossible d'ouvrir le fichier de l'exemplaire.",
              file=sys.stderr)
        sys.exit(1)
    ...
    consistency_result = check_consistency(
        resolution_data, instance_data[0], instance_data[1], args.nb_circonscriptions)
    if consistency_result != 0:
        ...
        if consistency_result[0] == 1:
            ...
            print("Raison : la municipalité " + str(consistency_result[3]) + " de la circonscription "
                  + str(consistency_result[2]) +
                  " de la solution " + str(consistency_result[1])
    ...
    distance_consistency_result = check_inner_district_distances(
        resolution_data, instance_data[0]*instance_data[1], args.nb_circonscriptions)
    ...
    objective, vote_proportions = compute_objective(
        resolution_data, instance_data[2], instance_data[0], instance_data[1], args.nb_circonscriptions)
    print("OK : la valeur de l'objectif de la dernière (ie, meilleure) solution fournie est de " + str(objective) + ".\n")
    print("n = " + str(instance_data[0]) + "*" + str(instance_data[1]
                                                     ) + " = " + str(instance_data[0]*instance_data[1]))
    ...
    if args.disp_mrz:
        ...
        print(objective, instance_data[0]*instance_data[1], args.nb_circonscriptions,
              args.exemplaire, ','.join(["{:.3f}".format(x) for x in vote_proportions]), sep=',')
