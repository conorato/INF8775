...
def _get_blocs_to_remove(current_solution, candidate):
    ...
    return (
        np.ones(len(bigger_width_indexes), dtype=np.bool_)
        if are_all_false else ~(bigger_width_indexes & bigger_depth_indexes))
...
def _update_tabous(new_tabous, tabous, candidates):
    ...
    if len(candidates_to_reinsert) != 0:
        candidates = np.vstack(
            (candidates, candidates_to_reinsert.reshape(-1, 3)))
...
def _update_time_to_live(updated_tabous):
    ...
    if len(candidates_to_reinsert) != 0:
        candidates_to_reinsert = np.array(candidates_to_reinsert)[
            0].reshape(-1, 3)
